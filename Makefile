...
GOBUILD_CLI=CGO_ENABLED=0 go build net/client/cli.go
...
windows-cli:
	GOARCH=amd64 GOOS=windows $(GOBUILD_CLI) -o $(BUILDIR_CLI)/$(NAME)-$@-amd64.exe
...
linux-cli:
	GOARCH=amd64 GOOS=linux $(GOBUILD_CLI) -o $(BUILDIR_CLI)/$(NAME)-$@-amd64.exe
